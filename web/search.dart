...
Future main() async {
  ...
  String strKeywords = (window.location.search.startsWith("?keywords="))
      ? window.location.search.substring("?keywords=".length)
      : "";
  ...
  if (keywords.isNotEmpty)
    where = generateWhereString(["name", "city", "country"], keywords);
  ...
  response = await Messenger.post(Request("get_rows", "events",
      {"columns": "*", "order_by": "name ASC", "where": where}));

  if (response.isEmpty) {
    ParagraphElement noEventsMessage = ParagraphElement();
  ...
  where = "1";
  if (keywords.isNotEmpty)
    where = generateWhereString(["name", "author", "format"], keywords);
  final DivElement booksContainer = querySelector("#books-container");
  response = await Messenger.post(Request("get_rows", "books", {
  ...
  if (response.isEmpty) {
    ParagraphElement noBooksMessage = ParagraphElement();
  ...
  response = await Messenger.post(Request("get_rows", "publications", {
    "columns": "id, name, date, url_pdf, author",
    "order_by": "date DESC",
    "where": where
  }));

  if (response.isEmpty) {
    ParagraphElement noPublicationsMessage = ParagraphElement();
...
String generateWhereString(List<String> columns, List<String> keywords) {
  ...
  columns.forEach((column) =>
      pairs.add("$column LIKE " + keywords.join(" OR $column LIKE ")));
