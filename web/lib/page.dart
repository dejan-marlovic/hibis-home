...
class Page {
  static final NodeValidator htmlValidator = NodeValidatorBuilder.common()
    ...
    ..allowNavigation(UriPolicyExternal());
  ...
  static Future init() async {
    if (panelRight != null) {
      ...
      ScriptElement twitterWidget = ScriptElement();
      ...
      Response response = await Messenger.post(Request(
          "get_rows", "publications", {
        "columns": "id, name, date, url_pdf",
        "order_by": "date DESC",
        "limit": "3"
      }));
    ...
    if (header != null) {
      ...
      hamburger.onClick.listen((_) {
        if (smallLinks.classes.contains("is-hidden"))
          smallLinks.classes.remove("is-hidden");
        else
          smallLinks.classes.add("is-hidden");
      });
    }
